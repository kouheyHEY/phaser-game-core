...
export class SoundManagerExample extends Phaser.Scene {
    ...
    setupVolumeControls() {
        ...
        categories.forEach((category, categoryIndex) => {
            this.add.text(100, 350 + categoryIndex * 50, `${category.toUpperCase()}:`, {
                fontSize: "14px",
                color: "#ffffff",
            });
            [0.0, 0.3, 0.7, 1.0].forEach((volume, volumeIndex) => {
                const button = this.add
                    .rectangle(200 + volumeIndex * 40, 350 + categoryIndex * 50, 30, 30, colors[categoryIndex])
                    .setInteractive();
                this.add
                    .text(200 + volumeIndex * 40, 350 + categoryIndex * 50, `${Math.round(volume * 10)}`, {
                    fontSize: "12px",
                    color: "#ffffff",
                })
                    .setOrigin(0.5);
...
export class AdvancedSoundExample {
     ...
     * 環境音の動的制御
     */
    updateAmbientSounds(environment) {
        // 現在の環境音を停止
...
// 設定例
export const soundManagerConfig = {
    type: Phaser.AUTO,
    width: 800,
    height: 600,
